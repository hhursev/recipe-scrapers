...
All of the example below come from https://www.bbcgoodfood.com/recipes/monster-cupcakes.
...
Returns the author of the recipe. This is typically a person's name, but can sometimes be an organisation or the name of the website the recipe came from. If the the recipe does not explicitly define an author, this should return the name of the website.
...
Returns the language of recipe page, as defined within the page's HTML. This is typically a two letter `str`, such as `en` for English or `de` for German, but may also include the dialect or variation, such as `en-US` for American English.
