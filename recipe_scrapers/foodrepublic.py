...
class FoodRepublic(AbstractScraper):
    ...
    def total_time(self):
        ...
        prep_time = int(prep_time_div.find('div', {'class': 'recipe-card-amount'}).text) if prep_time_div else 0
        cook_time = int(cook_time_div.find('div', {'class': 'recipe-card-amount'}).text) if cook_time_div else 0

        return prep_time + cook_time

    def yields(self):
        servings_div = self.soup.find('div', {'class': 'recipe-card-servings'})
        servings_amount = servings_div.find('div', {'class': 'recipe-card-amount'}).text if servings_div else '0'
