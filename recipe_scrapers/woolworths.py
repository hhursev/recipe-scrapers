...
class Woolworths(AbstractScraper):
    def __init__(
        ...
        if settings.TEST_MODE:  # when testing, we load a file
            ...
            url = 'https://foodhub.woolworths.com.au/content/woolworths-foodhub/en/{0}.model.json'.format(target)
            page_data = requests.get(
                url, headers=HEADERS, proxies=proxies, timeout=timeout
            ).content

        self.url = url
        page_data = json.loads(page_data).get(':items').get('root').get(':items').get('recipe_seo_data')
