...
class GreatBritishChefs(AbstractScraper):
    ...
    def total_time(self):
        ...
        tt1 = self.soup.find(
            'span',
            {'class': "RecipeAttributes__Time"}
        )
        if tt1:
            tt = tt1.find(
                'span',
                {'class': "header-attribute-text"}
            ).get_text()
    ...
    def yields(self):
        recipe_yield = self.soup.find(
            'span', {'class': "RecipeAttributes__Serves"}
        )
        if recipe_yield:
            yields = normalize_string(recipe_yield.find(
                'span', {'class': 'header-attribute-text'}).get_text())
            return yields
    ...
    def ingredients(self):
        ingredientsOuter = self.soup.find(
            'ul',
            {'class': 'IngredientsList__ListContainer'}
        )
        ingGroup = []
    ...
    def instructions(self):
        instructions = self.soup.find(
            'div', {'class': 'Method__List MethodList'})

        ins = instructions.findAll(
            'div', {'class': 'MethodList__StepText'})
    ...
    def description(self):
        d = normalize_string(self.soup.find(
            'div',
            {'class': 'RecipeAbstract__Abstract'}).text)
