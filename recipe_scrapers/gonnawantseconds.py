...
class GonnaWantSeconds(AbstractScraper):
    ...
    def image(self):
        image = self.soup.find(
            "div", {"class": "wprm-recipe-image wprm-block-image-normal"})  # , 'src': True})
        img = image.find(
    ...
    def ratings(self):
        ...
        try:
            r1 = self.soup.find(
                'div',
                {'class': 'wprm-recipe-rating'}
            )
            stars = r1.findAll('span',
                               attrs={'class': lambda e: e.endswith('wprm-rating-star-full') if e else False})
        except Exception:
            stars = []
        return len(stars)
