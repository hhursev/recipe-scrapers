...
class GialloZafferano(AbstractScraper):
    ...
    def ingredients(self):
        ingredients = self.soup.findAll("dd", {"class": "gz-ingredient"})

        if not ingredients:
            ingredients = self.soup.findAll("li", {"class": "gz-ingredient"})
    ...
    def ratings(self):
        return round(
            float(
                self.soup.find("div", {"class": "gz-rating-panel rating_panel"})
